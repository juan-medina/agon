...
// userID follows, ordered by ea.created_at descending with optional cursor
...
func GetFollowingActivity(ctx context.Context, pool *pgxpool.Pool, userID string, limit int, cursor string) ([]ActivityEvent, error) {
	...
	if cursor == "" {
		rows, err = pool.Query(ctx, `
			...
			JOIN follows f ON f.followee_id = ea.actor_id
		...
		rows, err = pool.Query(ctx, `
			...
			JOIN users a ON a.id = ea.actor_id
			JOIN users r ON r.id = e.recipient_id
			WHERE f.follower_id = $1 AND ea.created_at < $2
			ORDER BY ea.created_at DESC
			LIMIT $3
		`, userID, cursor, limit)
	}
